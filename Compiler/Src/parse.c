...
static ASTNode* parse_primary(Parser* parser) {
    ...
    switch (token.type) {
        ...
        case TOKEN_LITERAL_STRING:
            consume(parser);
            return create_literal(token.value,
                token.type == TOKEN_LITERAL_NUMBER ? NODE_LITERAL : NODE_LITERAL_LETTER);
...
static ASTNode* parse_if(Parser* parser) {
    ...
    if (match(parser, TOKEN_ELSE)) {
        ...
        if (match(parser, TOKEN_IF)) {
            expect(parser, TOKEN_LPAREN, "Se esperaba '('");
            ASTNode* condition_elseif = parse_expression(parser);
            expect(parser, TOKEN_RPAREN, "Se esperaba ')'");
            expect(parser, TOKEN_LBRACE,"se esperaba '{");
            while (!match(parser, TOKEN_RBRACE)) {
                ASTNode* stmt = parse_statement(parser);
    ...
    return create_if(condition,then_brach,then_count,elseif_branch,elseif_count,else_branch,else_count);
}
static ASTNode* parse_switch(Parser* parser) {
    ...
    while (!match(parser, TOKEN_LBRACE)) {
